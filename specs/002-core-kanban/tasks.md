...
- [X] T052 Add `@dnd-kit/core`'s `KeyboardSensor` to the `useSensors` call in `src/components/board/board-view.tsx` so drag & drop is keyboard-operable, per plan.md's Technical Context constraint (missing)
- [X] T053 Fix `inviteMember`'s `revalidatePath` call in `src/lib/actions/workspace.actions.ts` to target the real `/[workspaceSlug]/settings/members` route instead of the raw workspace id, per FR-003 (partial)
