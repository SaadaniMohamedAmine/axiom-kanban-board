...
import { ArchivedBoardList } from "./archived-board-list";
    ...
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-2">
        ...
        <Link href={`/${workspaceSlug}`} className="text-[13px] text-primary hover:text-primary/80 transition-colors">
          {tb("archivedPage.backToBoards")}
        </Link>
      </div>
      <p className="text-[13px] text-on-surface-variant mb-8">{tb("archivedPage.desc")}</p>

      {boards.length === 0 ? (
        <div className="border border-dashed border-outline-variant/40 rounded-xl p-12 text-center text-on-surface-variant">
          {tb("archivedPage.empty")}
        </div>
      ) : (
        <ArchivedBoardList boards={boards} canManage={canManage} />
      )}
    </div>
  );
}
